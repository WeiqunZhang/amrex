...
static std::ostream&
operator<< (std::ostream& os, Array<uint32_t,AMREX_SPACEDIM> const& morton)
{
    os << AMREX_D_TERM( '(' << morton[0] , <<
                        ',' << morton[1] , <<
                        ',' << morton[2])  << ')';
    if (os.fail())
        amrex::Error("operator<<(ostream&,const Array<uint32_t,AMREX_SPACEDIM)&) failed");
    return os;
}
...
namespace {
    ...
    SFCToken makeSFCToken (int box_index, IntVect const& iv)
    {
        ...
        uint32_t x = (iv[0] >= 0) ? static_cast<uint32_t>(iv[0]) + offset
            : static_cast<uint32_t>(iv[0]+std::numeric_limits<int>::max()) + 1u;
        uint32_t y = (iv[1] >= 0) ? static_cast<uint32_t>(iv[1]) + offset
            : static_cast<uint32_t>(iv[1]+std::numeric_limits<int>::max()) + 1u;
