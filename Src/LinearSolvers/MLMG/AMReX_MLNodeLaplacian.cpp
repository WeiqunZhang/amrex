                ...
                AMREX_HOST_DEVICE_PARALLEL_FOR_3D(bx, i, j, k,
                {
                    ...
                    for (int in = 0; in < AMREX_SPACEDIM; ++in) {
                        for (int it = 0; it < AMREX_SPACEDIM; ++it) {
                            if (it != in) {
                                if ((cell[in] < dlo[in] && lobc[in] == LinOpBCType::inflow) ||
                                    (cell[in] > dhi[in] && hibc[in] == LinOpBCType::inflow))
                                {
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
    ...
    AMREX_ALWAYS_ASSERT(has_rhcc == 0 || has_rhcc == m_num_amr_levels);
    ...
    for (int ilev = 0; ilev < m_num_amr_levels-1; ++ilev)
    {
        ...
        const int rr = AMRRefRatio(ilev);
        AMREX_ALWAYS_ASSERT(rr == 2 || rr == 4);

        const int ncomp = (has_rhcc) ? 2 : 1;
        frhs[ilev].reset(new MultiFab(amrex::coarsen(rhs[ilev+1]->boxArray(),rr),
                                      rhs[ilev+1]->DistributionMap(), ncomp, 0));
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
        ...
        for (MFIter mfi(*frhs[ilev],mfi_info); mfi.isValid(); ++mfi)
        {
            ...
            const Box& fvbx = amrex::refine(cvbx,rr);
...
#if (AMREX_SPACEDIM == 2)
            if (rr == 2) {
                AMREX_HOST_DEVICE_FOR_3D(cvbx, i, j, k,
                {
                    mlndlap_divu_fine_contrib<2>(i,j,k,fvbx,bx_vel,rhsarr,velarr,mskarr,fdxinv,
                                                 is_rz);
                });
            } else {
                AMREX_HOST_DEVICE_FOR_3D(cvbx, i, j, k,
                {
                    mlndlap_divu_fine_contrib<4>(i,j,k,fvbx,bx_vel,rhsarr,velarr,mskarr,fdxinv,
                                                 is_rz);
                });
            }
#else
            ...
            if (has_rhcc)
            {
                ...
                Array4<Real> rhsccarr(rhsarr, 1);
                if (rr == 2) {
                    AMREX_HOST_DEVICE_FOR_3D(cvbx, i, j, k,
                    {
                        mlndlap_rhcc_fine_contrib<2>(i,j,k,cc_fvbx,rhsccarr,rhccarr,mskarr);
                    });
                } else {
                    AMREX_HOST_DEVICE_FOR_3D(cvbx, i, j, k,
                    {
                        mlndlap_rhcc_fine_contrib<4>(i,j,k,cc_fvbx,rhsccarr,rhccarr,mskarr);
                    });
                }
            }
        }
    ...
    for (int ilev = m_num_amr_levels-2; ilev >= 0; --ilev)
    {
        ...
        const MultiFab& weight = *m_weight[ilev];
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
        ...
        for (MFIter mfi(*rhs[ilev],mfi_info); mfi.isValid(); ++mfi)
        {
            if (has_fine_bndry[mfi])
            {
                ...
                Array4<Real const> const& warr = weight.const_array(mfi);
                ...
                AMREX_HOST_DEVICE_FOR_3D(bx, i, j, k,
                {
                    mlndlap_divu_cf_contrib(i,j,k,rhsarr,velarr,crhsarr,rhccarr,
                                            cdmskarr,ndmskarr,ccmskarr, warr,
                                            is_rz,
                                            cdxinv,cccdom_p,cccdom_g,cnddom,lobc,hibc);
                });
#else
                AMREX_HOST_DEVICE_FOR_3D(bx, i, j, k,
                {
                                         ...
                                         const BoxArray& fine_grids, const IntVect& ref_ratio)
{
    ...
    // xxxxx TODO compSyncResidualCoarse refratio == 4
...
MLNodeLaplacian::reflux (int crse_amrlev,
                         ...
                         MultiFab& a_fine_res, MultiFab& fine_sol, const MultiFab&) const
{
    ...
    MultiFab fine_res_temp;

    if (amrrr == 2) {
        applyBC(crse_amrlev+1, 0, a_fine_res, BCMode::Inhomogeneous, StateMode::Solution);
    } else {
        fine_res_temp.define(a_fine_res.boxArray(), a_fine_res.DistributionMap(), 1, 3);
#ifdef AMREX_USE_OMP
#pragma omp parallel if (Gpu::notInLaunchRegion())
#endif
        for (MFIter mfi(fine_res_temp,TilingIfNotGPU()); mfi.isValid(); ++mfi) {
            Box const& vbx = mfi.validbox();
            Box const& tbx = mfi.growntilebox();
            Array4<Real const> const& srcfab = a_fine_res.const_array(mfi);
            Array4<Real> const& dstfab = fine_res_temp.array(mfi);
            AMREX_HOST_DEVICE_PARALLEL_FOR_3D(tbx, i, j, k,
            {
                if (vbx.contains(i,j,k)) {
                    dstfab(i,j,k) = srcfab(i,j,k);
                } else {
                    dstfab(i,j,k) = Real(0.0);
                }
            });
        }
        fine_res_temp.FillBoundary(m_geom[crse_amrlev+1][0].periodicity());
    }

    MultiFab const& fine_res = (amrrr == 2) ? a_fine_res : fine_res_temp;
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
    ...
    {
        const Box& cvbx = mfi.validbox();
        const Box& fvbx = amrex::refine(cvbx,amrrr);
        const Box& cc_fvbx = amrex::enclosedCells(fvbx);
        ...
        if (fsigma) {
            ...
            if (amrrr == 2) {
                AMREX_HOST_DEVICE_FOR_3D(cvbx, i, j, k,
                {
                    mlndlap_Ax_fine_contrib<2>(i,j,k,fvbx,cc_fvbx,farr,solarr,sigarr,marr,fdxinv,is_rz);
                });
            } else {
                AMREX_HOST_DEVICE_FOR_3D(cvbx, i, j, k,
                {
                    mlndlap_Ax_fine_contrib<4>(i,j,k,fvbx,cc_fvbx,farr,solarr,sigarr,marr,fdxinv,is_rz);
                });
            }
            ...
            if (amrrr == 2) {
                AMREX_HOST_DEVICE_FOR_3D(cvbx, i, j, k,
                {
                    mlndlap_Ax_fine_contrib<2>(i,j,k,fvbx,cc_fvbx,farr,solarr,sigarr,marr,fdxinv);
                });
            } else {
                AMREX_HOST_DEVICE_FOR_3D(cvbx, i, j, k,
                {
                    mlndlap_Ax_fine_contrib<4>(i,j,k,fvbx,cc_fvbx,farr,solarr,sigarr,marr,fdxinv);
                });
            }
#endif
        } else {
            ...
            if (amrrr == 2) {
                AMREX_HOST_DEVICE_FOR_3D(cvbx, i, j, k,
                {
                    mlndlap_Ax_fine_contrib_cs<2>(i,j,k,fvbx,cc_fvbx,farr,solarr,const_sigma,
                                                  marr,fdxinv,is_rz);
                });
            } else {
                AMREX_HOST_DEVICE_FOR_3D(cvbx, i, j, k,
                {
                    mlndlap_Ax_fine_contrib_cs<4>(i,j,k,fvbx,cc_fvbx,farr,solarr,const_sigma,
                                                  marr,fdxinv,is_rz);
                });
            }
#elif (AMREX_SPACEDIM == 3)
    ...
    const MultiFab& weight = *m_weight[crse_amrlev];
            ...
            if (csigma) {
                ...
                AMREX_HOST_DEVICE_FOR_3D(bx, i, j, k,
                {
                    mlndlap_res_cf_contrib(i,j,k,resarr,csolarr,crhsarr,csigarr,
                                           cdmskarr,ndmskarr,ccmskarr,fcocarr,warr,
