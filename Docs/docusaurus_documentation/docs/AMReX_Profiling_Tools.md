...
Currently, AMReX has two options for built-in profiling: `sec:tiny:profiling` and `sec:full:profiling`.
...
> [!NOTE]
> If you set `PROFILE = TRUE` (or `AMReX_BASE_PROFILE = ON`) to enable full profiling then this will override the `TINY_PROFILE` flag and tiny profiling will be disabled.
...
The output of TinyProfiler can help us to identify hot spots. For example, the following output shows the top three hot spots of a linear solver test running on 4 MPI processes.
...
The reason that the MPI communication appears slow is that the lightly loaded processes have to wait for messages sent by the heavily loaded processes. See also `sec:profopts` for a diagnostic option that may provide more insight on the load imbalance.

## Full Profiling

If you set `PROFILE = TRUE` then a `bl_prof` directory will be written that contains detailed per-task timings for each processor. This will be written in `nfiles` files (where `nfiles` is specified by the user). The information in the directory can be analyzed by the `sec:amrprofparse` tool within `sec:amrvis`. In addition, an exclusive-only set of function timings will be written to `stdout`.
...
## 2) A named, scoped timer, `BL_PROFILE_VAR`:
...
## 3) A named, scoped timer that doesn't auto-start, `BL_PROFILE_VAR_NS`:
...
> [!NOTE]
> The `_NS_` macro must, by necessity, also be a `_VAR_` macro. Otherwise, you would never be able to turn the timer on!

## 4) Designate a sub-region to profile, `BL_PROFILE_REGION`:
...
When using the Tiny Profiler, the only available region macro is the scoped macro. To create a region that profiles the <span class="title-ref">MyFuncs</span> code block, including all timers in the "Additional Code" regions, add macros in the following way:
...
> [!WARNING]
> Fortran functions cannot be profiled when using the Tiny Profiler. You will need to turn on the Full Profiler to receive the results from fortran instrumentation.
