...
in `AMReX_VisMF.H` and examining it with `Amrvis` (section `sec:amrvis`) can be helpful as well. In `AMReX_MultiFabUtil.H`, function
...
This default behavior can also be modified by applications, see for example [this custom application initializer](https://github.com/Exawind/amr-wind/blob/84f81a990152f4f748c1ab0fa17c8c663e51df86/amr-wind/main.cpp#L21).
...
- To help identify if there are race conditions, globally disable asynchronicity of kernel launches by setting `CUDA_LAUNCH_BLOCKING=1` or `HIP_LAUNCH_BLOCKING=1` in your environment variables. This will ensure only one kernel will run at a time. See the [AMD ROCm docs' chicken bits section](https://rocmdocs.amd.com/en/latest/Programming_Guides/HIP_Debugging.html#chicken-bits) for more debugging environment variables.
...
- Run under `gdb` with the [Intel Distribution for GDB](https://software.intel.com/content/www/us/en/develop/tools/oneapi/components/distribution-for-gdb.html).
